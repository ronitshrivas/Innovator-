...
class _LikeButtonState extends State<LikeButton>
  ...
  /// Only used to debounce rapid taps — never shown as a spinner to the user
  bool _isApiInFlight = false;
  ...
  void initState() {
    ...
    // Give the queue a reference to the service so it can flush old reactions
    HiveReactionQueue.instance.setService(widget.likeService);

    // Restore reaction state from what the API returned
    if (widget.initialLikeStatus) {
      _currentReaction =
          widget.initialReactionType != null
              ? ReactionTypeExtension.fromValue(widget.initialReactionType) ??
  ...
  //      │
  //      ▼
  //  [1] Optimistic update → UI changes instantly (NO spinner ever)
  //      │
  //      ▼
  //  [2] Check internet
  //      │
  //      ├── ONLINE ──► Call API
  //      │                 ├── SUCCESS → delete from Hive disk ✓
  //      │                 └── FAILURE → revert UI silently
  //      │
  //      └── OFFLINE ──► Save to Hive disk (survives app kill)
  //                        When internet returns → auto flush → API call
  //
  Future<void> _applyReaction(ReactionType? type) async {
    if (_isApiInFlight) return;
    ...
    // ── [1] Optimistic update — instant, user sees this immediately ──────────
    _updateLocalState(type, previous);
    widget.onLikeToggled?.call(_currentReaction != null);
    _isApiInFlight = true;
    ...
    try {
      ...
      if (!online) {
        // ── OFFLINE PATH ─────────────────────────────────────────────────────
        // Save to Hive disk — reaction will sync when internet returns
      ...
      if (result.success) {
        // Sync exact reaction type returned by server
        if (type != null && result.reactionType != null && mounted) {
          setState(() => _currentReaction = result.reactionType);
        }
        // Remove from Hive in case it was sitting there from a previous session
      ...
      // Network exception (timeout etc.) → treat as offline, save to Hive
  ...
  // ── Update count + reaction + play bounce animation ────────────────────────
  void _updateLocalState(ReactionType? newType, ReactionType? previous) {
    if (!mounted) return;
    setState(() {
      if (newType != null && previous == null) {
          ...
          child: Padding(
            ...
            child: Row(
              ...
              children: [
                ...
                // ── Icon — NO loading indicator ever ───────────────────────
                Stack(
                  clipBehavior: Clip.none,
                  children: [
                    emoji != null
                    ...
                    // Orange dot = saved to disk, waiting to sync with server
                    if (hasPending)
                      Positioned(
                        top: -2,
                        right: -2,
